...
function addToWeb(p, ele, returnString) {
	...
	var price = `<strong>` + numToString(p.price) + `&#8363;</strong>`;
	if (p.promo && p.promo.name == "giareonline") {
		// khuyến mãi 'Giá rẻ online' sẽ có giá thành mới
		price = `<strong>` +numToString(p.promo.value) + `&#8363;</strong>
				<span>` + numToString(p.price) + `&#8363;</span>`;
	}
	...
	`<li class="sanPham">
		<a href="` + chitietSp + `">
			...
			<div class="tooltip">
				<button class="themvaogio" onclick="themVaoGioHang('`+p.masp+`', '`+p.name+`','`+p.price+`'); return false;">
