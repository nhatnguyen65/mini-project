...
router.post('/add', authMiddleware, async (req, res) => {
    try {
        ...
        if (!productId || !soLuong) return res.status(400).json({ success: false, message: 'Thiếu productId hoặc soLuong' });
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const cart = await Cart.findOne({ user: userId })
                               .populate('products.product', 'name price img promo');
...
router.put('/', authMiddleware, async (req, res) => {
    try {
        ...
        if (!products) return res.status(400).json({ success: false, message: 'Thiếu products' });
