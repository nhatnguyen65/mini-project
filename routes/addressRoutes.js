...
router.get('/default', authMiddleware, async (req, res) => {
    try {
        const address = await Address.findOne({ userId: req.session.userId, macDinh: true });
...
router.put('/:addressId', authMiddleware, async (req, res) => {
    try {
        ...
        if (address.userId.toString() !== req.session.userId) {
            return res.status(403).json({ message: 'Không có quyền chỉnh sửa địa chỉ này' });
        }

        const updated = await Address.findByIdAndUpdate(req.params.addressId, req.body, { new: true });
        const user = await User.findById(req.session.userId).populate('diaChi', '-userId');
        res.json(user);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.delete('/:addressId', authMiddleware, async (req, res) => {
    try {
        ...
        if (!address) return res.status(404).json({ message: 'Không tìm thấy địa chỉ' });
        if (address.userId.toString() !== req.session.userId) {
            return res.status(403).json({ message: 'Không có quyền xóa địa chỉ này' });
        }
